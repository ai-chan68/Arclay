...
export async function getMessagesByTaskId(taskId: string): Promise<Message[]> {
  ...
  if (database) {
    return database.select<Message[]>(
      // created_at in SQLite uses second-level precision; rely on monotonic id for stable stream order
...
export async function countMessagesByTaskId(taskId: string): Promise<number> {
  const database = await getSQLiteDatabase();

  if (database) {
    const result = await database.select<{ count: number | string }[]>(
      'SELECT COUNT(*) AS count FROM messages WHERE task_id = $1',
      [taskId]
    );
    const rawCount = result[0]?.count ?? 0;
    return typeof rawCount === 'string' ? Number.parseInt(rawCount, 10) || 0 : rawCount;
  }
