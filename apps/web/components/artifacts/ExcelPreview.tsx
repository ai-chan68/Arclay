 ...
 *
 * Excel/XLSX spreadsheet preview with xlsx.js
 */

import React, { useEffect, useState } from 'react';
import { FileSpreadsheet, ExternalLink, Loader2 } from 'lucide-react';
import type { ExcelSheet, PreviewComponentProps } from '../../shared/types/artifacts';
...
async function readFileViaAPI(path: string): Promise<Uint8Array> {
  ...
  const sizeResponse = await apiFetchRaw('/api/sandbox/read', {
    method: 'POST',
  ...
  if (!sizeResponse.ok || !sizeResult.success) {
    throw new Error(sizeResult.error || 'Failed to read file');
  }

  // Decode base64 to binary
  const base64Data = sizeResult.content;
    ...
    async function loadExcel() {
      ...
      try {
        ...
        if (arrayBuffer.byteLength > MAX_PREVIEW_SIZE) {
          console.log('[Excel Preview] File too large:', arrayBuffer.byteLength);
