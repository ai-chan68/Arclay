...
│  │ apps/web                     │                                    │     │
│  │ - Task UI / Message Stream   │                                    ▼     │
│  │ - Planning / Approval UI     │                    ┌────────────────────┐ │
│  │ - Settings / Notes           │                    │ API Sidecar        │ │
│  └──────────────┬───────────────┘                    │ apps/agent-service │ │
│                 │                                    │ - Agent runtime    │ │
│   Tauri IPC /   │                                    │ - Plan / Execute   │ │
│   Plugins       │                                    │ - Sandbox / MCP    │ │
│                 ▼                                    │ - Skills routing   │ │
│  ┌──────────────────────────────┐                    └────────────────────┘ │
│  │ Rust / Tauri Runtime         │                               ▲           │
│  │ apps/desktop                 │                               │           │
│  │ - Window / app lifecycle     │     spawn / supervise sidecar │           │
│  │ - get_api_port IPC           │ ───────────────────────────────┘           │
...
- `apps/web`
  - 用户界面与任务状态管理。
  - 通过 HTTP/SSE 调用 `apps/agent-service`，承接 planning / approval / execution 流。
  - 在桌面模式下通过 Tauri IPC 读取 sidecar 端口，并在首次连库前通过 `wait_for_db_ready` 等待迁移完成，再通过 `@tauri-apps/plugin-sql` 访问 SQLite。
- `apps/agent-service`
  - 真正的 Agent Harness 与运行时核心。
  - 持有规划、审批、执行、运行时门控、Skills 路由、MCP 装配、Sandbox 策略。
- `apps/desktop`
  - 桌面外壳与本机能力边界。
  - 启动时初始化数据库、拉起并监管 sidecar、暴露 IPC 与插件能力给前端。
  - 不承载 Agent 业务编排，主要负责桌面生命周期与本机资源接入。
...
1. Rust 异步启动 SQLite 初始化与迁移。
2. 根据环境决定是否自动拉起 bundled `arclay-api` sidecar。
3. 如果检测到兼容的桌面 sidecar 已经运行，可跳过 bundled sidecar 启动。
4. 监听 sidecar stdout，解析实际监听端口。
5. 通过 `get_api_port` IPC 提供给前端。
6. 前端首次访问 SQLite 前，会通过 `wait_for_db_ready` IPC 等待数据库初始化完成。

补充说明：

- 数据库初始化与 sidecar 判定/拉起在启动阶段是并行发生的，不是严格串行。
- debug 开发模式下默认不自动拉起 sidecar，除非显式开启 `ARCLAY_SPAWN_SIDECAR_IN_DEV`。
- 当前数据库 ready 契约已经补齐到主数据库入口：前端不会在首次真正打开 SQLite 前跳过 Rust 侧 migration 完成信号。
...
- 当前主路径是前端在通过 `wait_for_db_ready` 确认迁移完成后，再用 `@tauri-apps/plugin-sql` 直接打开 `sqlite:arclay.db`。
- 这也意味着 UI 数据边界已经可以定为：`apps/web` 是唯一 UI 数据访问层，`apps/desktop` 不再承担 UI CRUD 入口。

因此当前运行时实际上有两个持久化平面：

- UI 数据库平面（SQLite / Web 模式下的 IndexedDB）：会话、任务、消息、文件、设置、预览实例。
- sidecar 文件平面（JSON/JSONL/工作目录文件）：计划审批状态、turn runtime、历史流水、memory、turn detail、artifacts 上下文。
...
| Tauri plugins | UI ↔ Rust/plugin | SQLite / FS / Dialog 等 | 桌面能力封装，前端通常以 async API 方式调用 |
...
因此桌面侧当前保留的数据库相关 IPC 只剩 `wait_for_db_ready`。

按当前代码核验结果：

- `db_execute` / `db_query` 已无实际调用面。
- 它们属于已经删除的遗留 SQL IPC。
- 当前 UI 数据主路径就是 `wait_for_db_ready` + `plugin-sql`。
...
- 先孤儿化未完成审批。
- 再清扫 `planStore`：把执行中的 plan 收敛为 `orphaned`，把超时待审批 plan 收敛为 `expired`。
- 若存在过期 plan，再取消关联 turn。
- 最后清扫 `turnRuntimeStore`：把中断中的 runtime / turn 收敛到 `idle` / `failed` / `cancelled`。
...
- 恢复不是“单一真相源重建全局”，而是 approval / plan / turn 三套状态分别清扫后再由前端合并展示。
- 前端页面恢复时，pending approval / clarification 的 phase 优先级高于 turn runtime 映射。
- 任务列表中的 `task.status` 合并时，approval terminal 与 turn terminal 的优先级高于数据库里已有的终态字段。

恢复优先级可以收敛成下面这张表：

| 场景 | 优先看的状态 | 次级状态 | 说明 |
|---|---|---|---|
| sidecar 启动恢复 | approval / plan / turn 各自清扫 | 无单一总源 | 不是“一个状态源重建全部”，而是多套状态分别收敛 |
| 页面 phase 恢复 | pending approval / clarification | turn runtime 映射 | 有 pending 交互时，前端 phase 不会被 turn state 覆盖 |
| 任务终态合并 | approval terminal | turn terminal → DB `task.status` → 消息推导 | `resolveTaskStatus(...)` 先看审批打断，再看 turn 终态，再看数据库已有终态 |
| plan 的作用 | pending plan / 执行控制 | 不直接决定 `task.status` | `plan` 主要影响是否还能恢复待审批计划，以及后续执行控制 |

因此当前约定应理解为：

- `plan` 更接近审批与执行控制状态。
- `turn` 更接近一次具体运行的生命周期状态。
- UI `task.status` 是前端对 approval / turn / persisted task 的合并投影，不是 sidecar 文件状态的直接镜像。
...
- 当前运行时路由候选仍来自项目内 `SKILLs/` 扫描结果。
- “外部来源管理”更像来源配置、安装、更新、修复的入口，不代表运行时会直接从外部源扫描并注入 skills。
- `sources` 主要通过“把 skill 导入/更新进项目 `SKILLs/`”间接影响运行时候选，而不是直接充当路由开关。
...
- 如果模型没有给出 `deliverableType`，执行入口会把它视为无效并写回 `unknown`。

补充：

- 只有在 `deliverableType` 缺失时，执行入口才会退回 legacy 关键字判定。
- 一旦 plan 已经显式带着 `unknown` 进入执行阶段，就不会再走“缺失类型”的 legacy 兜底分支。
...
- `local_service` / `deployed_service`
  - 启用严格 runtime gate。
- `unknown`
  - 当前默认不自动启用严格 runtime gate。
  - 当前实现里它更像“类型未知”的占位值，而不是“自动进入服务级健康检查”。
  - 因为它已经是一个显式类型值，所以不会再触发“缺失类型”时的 legacy runtime intent 判定。
  - 这意味着 `unknown` 主路径下通常不会进入 `evaluateRuntimeGate(...)`。
...
- 先通过 `wait_for_db_ready` 等待数据库 ready，再通过 `plugin-sql` 访问 SQLite。
