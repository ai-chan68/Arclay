...
本次重构依据团队 harness 工程实践文档，对 EasyWork 后端 Agent 执行层进行系统性对齐。核心认知：
...
```
~/.easywork/metrics/YYYY-MM.jsonl
```
...
```bash
# 本月 pass@1 率
jq 'select(.attempt==1) | .success' ~/.easywork/metrics/2026-03.jsonl | sort | uniq -c

# 平均耗时
jq '[.durationMs] | add/length' ~/.easywork/metrics/2026-03.jsonl
```
